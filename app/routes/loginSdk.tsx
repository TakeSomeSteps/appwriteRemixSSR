...
import { SDK_AUTH_NAME_PASS_STRATEGY } from "~/auth";
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  const authResponse = await authenticator.authenticate(SDK_AUTH_NAME_PASS_STRATEGY, request, {
...
export default function Index() {
  ...
  return (
    <>
      <h1 className="text-2xl font-bold text-center mb-4 dark:text-gray-200">Login using SDK</h1>
      <LoginFormPostBackend postError={loaderData} routeToPost="loginSdk" />
    </>
  );
}
